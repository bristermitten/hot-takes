...
function hotTakeImage(thing: HotTakeThing): string {
...
export default async function generateHotTake(): Promise<HotTakeResponse> {
	...
	const takeImage = hotTakeImage(randomTake)
	if (takeImage) images.push(takeImage)
	...
	const take = takeValue.replace(/{[\w|]+}/g, value => {
			...
			const replacementImage = hotTakeImage(randomReplacement)
